/*
 * @adv
 * 4.3.0 初始化代码，基于4.2.X的改进优化版本，默认还是以v2方式发送信息
 * 4.3.1 支持解析mpyck参数，以mc属性发送给后台，用于dmp匹配点击
 * @version 4.3.1
 * */
(function (W,D,E) {
  try{
    ...
    var objTypeArg = /\[object (Boolean|Number|String|Function|Array|Date|RegExp)\]/,
        ...
        stringifyItem = function (obj,objType) {//模拟JSON.stringify方法，stringifyItem为其递归
            var str=objTypeString(obj)=="array"?"[":"{",property,value;
            for (property in obj)
              if (hasOwnProperty(obj, property)) {
                value = obj[property];
                if(objTypeString(value)=="array"||objTypeString(value)=="object"){
                  str+=objType=="array"?stringifyItem(value,objTypeString(value)) +",":"\""+property+"\":"+stringifyItem(value,objTypeString(value)) +","
                }else if(objTypeString(value)=="string"){
                  str+=objType=="array"?"\""+value +"\",":"\""+property+"\":\""+value +"\",";
                }else {
                  str+=objType=="array"?""+value +",":"\""+property+"\":"+value +",";
                }
              }
          str+=objTypeString(obj)=="array"?"]":"}";
            return str;
        },
        stringify=function(obj){//模拟JSON.stringify方法，stringifyItem为其递归
            var str  = objTypeString(obj)=="array"||objTypeString(obj)=="object"?stringifyItem(obj,objTypeString(obj)).replace(/,}/g,"}").replace(/,]/g,"]"):obj
            return (str!="{}"?str:"");
        },
        ...
        globalEval = function( code ) {// Evaluates a script in a global context
          var script,code = trim( code );
          if (code) {
              //统一使用script的方法
              ...
              script.text = code;
              (D.body || D.head).appendChild( script ).parentNode.removeChild( script );
          }
        },
        ...
        getBrowserData=function(browserInfo){//初始化参数代码，用来收集广告主传递的需要回传到服务端的数据
            var browserInfo={},nav =W.navigator, screen = W.screen;
            screen && (browserInfo["sr"] = screen.width + "x" + screen.height);//screenResolution
            screen && (browserInfo["sc"] = screen.colorDepth + "-bit");//screenColors,得到颜色深度
            browserInfo["fv"] = getFlashVersion();//flashVersion,得到flash版本
            browserInfo["je"] = (nav && "function" === typeof nav.javaEnabled && nav.javaEnabled() || !1);//javaEnabled,是否支持java
            browserInfo["lg"]=(nav && (nav.language || nav.browserLanguage) || "").toLowerCase();//language,浏览器语言
            browserInfo["vb"]=pageVisibility.support?"1":"0";//是否支持可视监测属
        ...
        getPageData=function(){
            ...
            pageInfo["vp"] = c;//viewportSize
            pageInfo["tt"]=D.title;
            pageInfo["ec"] = D.characterSet || D.charset;//encoding,获取encoding
            pageInfo["u"]= url
            pageInfo["r"]= referrer;//获取referrer
            if(pageVisibility.support)pageInfo["vbt"]=pageVisibility.hidden?pageVisibility.total:pageVisibility.sumTime();//vbt
            pageInfo["sp"]=max_sp;//sp参数，最大滚动条百分比
            return pageInfo;
        },
        ...
        //记录当前页面的最大滚动条百分比
        max_sp=0,
        scrollEventFLag=!1,//滚动事件触发标记
        _scrollPercent = function(){
          ...
          if(scrollHeight>clientHeight){
              ...
              max_sp=100;scrollEventFLag=true;return max_sp;
          }
          if(max_sp<scrollPercent)max_sp=scrollPercent;
          return scrollPercent;
        },
        scrollPercent = function(){
          var scrollPercentNum =_scrollPercent();
          if(max_sp>=100)removeEventListener(W,"scroll",scrollPercent);
          //超过百分之80发送事件
          if(!scrollEventFLag&&scrollPercentNum>80){
              globalEval(commandName+"('event','scrollEvent')");
                ...
                if(z){
                    if(z==true){
                        ...
                        for(var k=0;k<z.length;k++){
                            (function(){
                                var j = s;
                                addEventListener(z[k],"click",function(){try{
        ...
        urlReg=function(t,s){//url匹配正则的处理
            var c = (t.r==1?pageInfo["u"]:pageInfo["r"]);
            if(t.o==1){
                var rS = t.v,
                    reg = new RegExp(rS),
                    r = reg.exec(c);
            ...
            try{
                ...
                if("event"==c){
                    if(arg.t){
                        ...
                        if(void 0!=ti && ""!=ti){//如果转化点id不为空，则为转化请求
                            urlType = "cvt";
                        }
                    }
                    //开始发送send请求
                    ...
                    for(var i=0;i<ds.length;i++){
                      var adv=("https:" == location.protocol ? "https" : "http") + "://" + ds[i] + "/"+urlType+"?a="
                      if (version == 2) {
                        var c,s,pi,p,u = pageInfo.u,r = pageInfo.r,pv,OrderNo,Money,ProductList,browserInfoString="",e="",ce="";
                        pv = userInfo.ca;
                        delete userInfo.ca;
                        for(var prop in browserInfo){
                          if(void 0 !=browserInfo[prop])browserInfoString +="&"+prop+"="+E(browserInfo[prop]);
                        }
                        delete pageInfo.u;
                        delete pageInfo.r;
                        delete pageInfo.tt;
                        browserInfo = conversionV3toV2(browserInfo);
                        userInfo = conversionV3toV2(userInfo);
                        siteInfo = conversionV3toV2(siteInfo);
                        pageInfo = conversionV3toV2(pageInfo);
                        if(eventName=="viewItem"){
                            if("pn" in eventParams){
                                p = eventParams.pn;
                            }
                          var pis = eventParams.pis&&eventParams.pis.replace("x","X").split("X")
                          var ms = eventParams.ms&&eventParams.ms.replace("x","X").split("X")
                          delete eventParams.pis;//宽x高
                          delete eventParams.ms;//宽x高

                          if(pis&&pis.length==2){
                            eventParams.pw = pis[0];
                            eventParams.ph = pis[1];
                          }
                          if(ms&&ms.length==2){
                            eventParams.mw = ms[0];
                            eventParams.mh = ms[1];
                            ...
                            // 部分细节参数不再处理
                            pi = stringify(eventParams);
                            eventParams = {};
                        }else if(void 0!=ti && ""!=ti && (eventName=="order" || eventName=="viewCart" || eventName=="purchase")){
                          if("m" in eventParams){
                            Money = eventParams.m;
                            delete eventParams.m;
                          }
                          if("i" in eventParams){
                            OrderNo = eventParams.i;
                            delete eventParams.i;
                          }
                          if("it" in eventParams){
                            ProductList = "";
                            for(var j = 0;j<eventParams.it.length;j++){
                              var _p = eventParams.it[j];
                              ProductList +=(_p.id?_p.id:"")+','+(_p.count?_p.count:"")+','+(_p.price?_p.price:"")+';';
                            }
                            delete eventParams.it;
                          }

                        }else if (eventName=="leads" || eventName=="custom" || eventName=="register"){
                          if("i" in eventParams){
                            OrderNo = eventParams.i;
                            delete eventParams.i;
                          }
                          if("d" in eventParams){
                            ProductList = "";
                            for(var par in eventParams.d){
                              ProductList +="&"+par+"="+eventParams.d[par];
                            }
                            ProductList=ProductList.substring(1);
                            delete eventParams.d;
                          }
                          if(eventName=="custom"){
                            if("ce" in eventParams){
                              ce = eventParams.ce;
                              delete eventParams.ce;
                            }
                          }
                        }
                        eventParams= conversionV3toV2(eventParams);
                        if(browserInfoString.length>0){
                          e += browserInfoString.substring(1);//浏览器参数
                        }
                        for(var prop in pageInfo){
                          if(void 0 !=pageInfo[prop])e +="&"+prop+"="+E(pageInfo[prop]);
                        }
                        e +="&ur=" + E(stringify(userInfo));
                        e +="&st=" + E(stringify(siteInfo));
                        e +="&ev=" + E(eventInfo[eventName].prop.shortName);
                        if(!isEmptyObject(eventParams))e +="&ep=" + E(stringify(eventParams));
                        e +=(""!=ce?("&ce="+ce):"");
                        if(void 0!=ti && ""!=ti) e +=(pv?("&pv="+pv):"");//由于v2转化没有pv参数，所以放在扩展里

                        adv += (void 0!=ti && ""!=ti?E(ti):E(py.a))+ // a参数,转化请求时使用转化点id代替a参数
                          (p?("&p=" + E(p)):"")+// 单品信息，多个单品使用逗号分隔
                          (pi?("&pi=" + E(pi)):"")+// 单品访客收集商品信息
                          (ipy.getInfo("ipycookie")?("&c="+ipy.getInfo("ipycookie")):"")+//c参数
                          (ipy.getInfo("ipymcookie")?("&mc="+ipy.getInfo("ipymcookie")):"")+//mc参数
                          (j == 0?"":"&s="+j)+//s参数,jump的cookie
                          (u?("&u=" +E(u)):"")+// 当前页面url
                          (r?("&r=" + E(r)):"")+// refer url
                          (((void 0 == ti || ti == "") && pv)?("&pv=" + E(pv)):"")+// // 动态分类参数，多个分类使用逗号分隔
                          "&rd=" + (new Date()).getTime()+ //rd参数，时间戳
                          (Money?("&Money=" + E(Money)):"")+// Money
                          (OrderNo?("&OrderNo=" + E(OrderNo)):"")+// OrderNo
                          (ProductList?("&ProductList=" + E(ProductList)):"")+// ProductList
                          "&v=2" +//v参数
                          ((void 0==e)?"":"&e="+E(e)); // 扩展参数
                        log(adv.substring(4));
                      } else {
                        adv += E(py.a) +  //a参数,广告主ID
                          ...
                          "&rd=" + (new Date()).getTime(); //rd参数，时间戳
                        log(adv.substring(4));//后台没有正式上线前，不直接发送请求过去
                      }
                      createScript(adv);
                    }
                }
        ...
        //处理mpyck,如果命名冲突，还是以clickParam设置加m前缀，使用ipymcookie存储信息
        _setIpyMcookie = function(){
            var ck = "m"+_getClickParam();
            var d= ipy.getQueryString(ck)||ipy.getReferrerQueryString(ck);
            d = d ? d:ipy.getInfo("ipymcookie");
            ipy.setInfo('ipymcookie',d);//clickParam的cookie存储
        },
        ...
        delayExecution = function (fun) {
          W.setTimeout(fun, 0)
        },
        isReady = !1,//false
        readyFunctionArray = [_scrollPercent],
    ...
    W.ipy = {
      ...
      setJumpSession:function(){
          var c = ipy.getInfo("ipycookie")||ipy.getInfo("ipymcookie");
          if (c && c != null) {
              var j = ipy.getJump();
              ipy.setJump(++j);
          };
     ...
     * @param obj
     */
    var conversionV3toV2 = function(obj){
    ...
    var browserInfo=getBrowserData(),//获取浏览器信息
        ...
        eventInfo={};
    // 获取发送请求的版本
    var a = py.a, version3List = [], comId = a && a.split(".")[0], version = 2;
    for (var i = 0; i < version3List.length; i++) {
      if (version3List[i] == comId) {
        version = 3;
        break;
      }
    }
    ...
    eventInfo["viewCart"] = new _eventObject("viewCart","vc").addProps([["id","i"],["money","m"],["items","it"],["count","ct"],["price","pr"],["trackId","ti"]]);
    ...
    eventInfo["custom"] = new _eventObject("custom","cm").addProps([["customEvent","ce"],["id","i"],["data","d"],["trackId","ti"]]);
    ...
    var defaultEventsInit = function(){
      ...
      var events = eventInfo["default"].get("events");
      //发送用户停留时间
      if(pageVisibility.support && hasValue(events,"standingTime")){
        var pvtimer=null;
        if(!pageVisibility.hidden){
          pvtimer=setTimeout(commandName+"('event','standingTime')",2e4);
        }
        pageVisibility.change(function(){
          clearTimeout(pvtimer);
          if(this.visibilityState == "visible"){
            pageVisibility.visibilityTime = new Date();
            var t=2e4-pageVisibility.total;
            t>0&&(pvtimer=setTimeout(commandName+"('event','standingTime')",t));
          } if(this.visibilityState == "hidden"){
            pageVisibility.sumTime();
          }
        });
      }
      if(!scrollEventFLag && hasValue(events,"scrollEvent"))addEventListener(W,"scroll",scrollPercent);//当标识不为true时，绑定滚动条事件
    }
    ...
    py[callback] = function(cmd,cvd){
      try{
          ...
          log("----执行后台配置的代码结束---------");
          _setIpycookie();//后台也有可能配置了clickParam命令，需要再次设置
          _setIpyMcookie();//设置ipymcookie,后台也有可能配置了clickParam命令，需要再次设置
    ...
    if(py.r){//当设置为true时，则不需要进行cookie mapping和presadv请求
      ...
      log("请求后台方法-------------------");
      createScript(("https:"==location.protocol?"https" : "http") + "://stats.ipinyou.com/presadv?a=" + E(py.a) + "&cb=" + commandName + "." +callback,function(){
        log("请求后台执行成功");
      },defaultRunning);
    }
